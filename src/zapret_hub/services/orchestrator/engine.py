...
class OrchestratorEngine:
    ...
    def _passive_scan(self) -> None:
        ...
        if best is None:
            best = self._configured_site_incident(settings, learner, lists_dirs)
        if best is None:
            return
        self._handle_incident(best)

    def _configured_site_incident(
        self, settings: Any, learner: HostlistLearner, lists_dirs: list[Path]
    ) -> dict[str, Any] | None:
        """Probe user-declared sites whose connections cannot be identified by netstat."""
        now = time.monotonic()
        if now - self._last_site_scan_at < _SITE_SCAN_INTERVAL_S:
            return None
        self._last_site_scan_at = now
        probe = getattr(self._signals, "probe_host_access", None)
        if not callable(probe) or self.context is None:
            return None
        install_root = getattr(self.context.paths, "install_root", None)
        template = Path(install_root) / "sample_data" / "auto_sites.json" if install_root else None
    ...
    def _handle_incident(self, incident: dict[str, Any]) -> None:
        ...
        try:
            ...
            steps = self._tuner.plan(
                ...
                allow_strategy_after_list=bool(incident.get("allow_strategy_after_list")),
            )
            if not steps:
                if knowledge is not None:
