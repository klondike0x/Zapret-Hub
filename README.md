...
### Перенос данных portable-версии

Portable-версия хранит настройки и рабочие данные в папке `user_data` рядом с `Zapret_Hub.exe`, поэтому её можно переносить вместе с этой папкой.

Новая portable-копия не импортирует данные обычной установки из %LOCALAPPDATA%\Zapret_Hub. Если нужно перенести старые данные в portable-версию, перед первым запуском создайте рядом с EXE пустой файл `portable_legacy.flag`. Альтернативно можно запустить приложение с переменной окружения `ZAPRET_HUB_MIGRATE_LEGACY_DATA=1`.
